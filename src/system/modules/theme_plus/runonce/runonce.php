...
class ThemePlusRunonce extends Frontend
{
	...
	{
		if ($this->Database->tableExists('tl_additional_source', false, true) && !$this->Database->fieldExists('additional_source', 'tl_layout', true)) {
	...
	protected function upgrade2_0()
	{
		...
		if ($this->Database->tableExists('tl_additional_source', false, true)) {
			...
			if (!$this->Database->tableExists('tl_theme_plus_file', false, true)) {
				try {
  ...
  KEY `pid` (`pid`)
			...
			if ($blnTableThemePlusFileExists && $this->Database->tableExists('tl_additional_source', false, true)) {
				try {
